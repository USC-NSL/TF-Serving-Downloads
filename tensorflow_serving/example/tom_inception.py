...
tf.app.flags.DEFINE_string('checkpoint_dir', '/home/yitao/Downloads/test/20180623/inception-v3',
...
def export():
  ...
  with tf.Graph().as_default():
    ...
    # serialized_tf_example = tf.Print(serialized_tf_example, [serialized_tf_example], "[Yitao] debug...1")
    feature_configs = {
    ...
    # jpegs = tf.Print(jpegs, [jpegs], "[Yitao] debug...2")
    ...
    with tf.Session() as sess:
      ...
      # Yitao-TLS-Begin
      data = open("/home/yitao/Documents/TF-Serving-Downloads/dog.jpg", "rb").read()
      # data = f.read()
      # print(sess.run(classes, feed_dict={serialized_tf_example:tf.contrib.util.make_tensor_proto(data, shape=[1])}))
      
      tmp = tf.contrib.util.make_tensor_proto(data)
      
      # tmp = tf.train.Features(
      #   feature={
      #     "image/encoded":_bytes_feature(data)
      #   })
      
      # tmp = tf.train.Example(
      ...
      print(sess.run(classes, feed_dict={serialized_tf_example:tmp}))
      # Yitao-TLS-End
