...
class OlympianMaster(tomtest_grpc_pb2.OlympianMasterServicer):
  # def callServer(input_path):
  #   # host, port = FLAGS.server.split(':')

  def CallTest(self, request, context):
    print("Master's function is remotedly called with input_path = %s!" % request.input_path)

    # callServer(request.input_path)
    host = "localhost"
    port = "9000"
    channel = implementations.insecure_channel(host, int(port))
    stub = prediction_service_pb2.beta_create_PredictionService_stub(channel)
...
def main(_):
  ...
  tomtest_grpc_pb2.add_OlympianMasterServicer_to_server(OlympianMaster(), server)
